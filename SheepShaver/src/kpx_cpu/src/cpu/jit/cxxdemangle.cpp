...
SYNOPSIS
	...
	static int
	demangle_class (work_stuff *work, const char **mangled,
			strint *declp)
...
SYNOPSIS
	...
	static int
	demangle_prefix (work_stuff *work, const char **mangled,
			 string *declp);
...
SYNOPSIS
	...
	static int
	gnu_special (work_stuff *work, const char **mangled,
		     string *declp);
...
SYNOPSIS
	...
	static int
	demangle_qualified (work_stuff *, const char *mangled,
...
static int
do_type (work_stuff *work, const char **mangled, string *result)
{
  int n;
  int done;
  int success;
...
static int
demangle_fund_type (work_stuff *work, const char **mangled, string *result)
{
  int done = 0;
  int success = 1;
  char buf[10];
...
do_hpacc_template_const_value (
	work_stuff *work ATTRIBUTE_UNUSED,
	const char **mangled,
	string *result)
{
  int unsigned_const;
...
static int
register_Btype (work_stuff *work)
{
  int ret;
...
static void
forget_B_and_K_types (work_stuff *work)
{
  int i;
...
int main(int argc, char *argv[])
{
	for (int i = 1; i < argc; i++) {
		const char *mangled_name = argv[i];
		char *demangled_name = cxx_demangle(mangled_name, NULL, NULL, NULL);
		if (demangled_name == NULL)
			printf("Could not demangle string '%s'\n", mangled_name);
		else {
